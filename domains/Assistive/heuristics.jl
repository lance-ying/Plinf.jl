...
import SymbolicPlanners: compute, precompute!, is_precomputed
...
function extract_action_costs(spec::Specification, agents::Vector{Const})
    ...
    for agent in agents
        ...
        noop_cost =
            get_action_cost(spec, Compound(:noop, Term[agent]))
        pickup_cost =
    ...
    return (move_costs, noop_costs, pickup_costs, unlock_costs)
end

function extract_min_action_costs(spec::Specification, agents::Vector{Const})
    move_costs, noop_costs, pickup_costs, unlock_costs =
        extract_action_costs(spec, agents)
    return (minimum(move_costs), minimum(noop_costs),
...
function compute(h::GoalManhattan,
    ...
    move_costs, noop_costs, _, _ = extract_action_costs(spec, h.agents)
...
function compute(h::DoorsKeysMSTHeuristic,
    ...
    goal_clauses = decompose_goals(domain, state, spec)
    if isempty(goal_clauses) return 0.0 end
    # Extract action costs
    move_cost, noop_cost, pickup_cost, unlock_cost =
        extract_min_action_costs(spec, h.agents)
    # Consruct location graph
...
function find_necessary_keys_and_doors(
    ...
    for r in r_goals[2:end]
        ...
        for (kd1, kd2) in Iterators.product(kd_sets, new_kd_sets)
            ...
            length(new_keys) < length(new_doors) && continue
...
function find_necessary_keys_and_doors(
    ...
    for item in items
        x, y = get_obj_loc(state, item)
        (x < 0 || y < 0) && continue # Ignore held or unreachable items
...
function find_necessary_keys_and_doors_recursive(
    ...
    for _ in 1:max_iters
        new_kd_sets = nothing
        keys_added = false
        for (i, (nec_keys, nec_doors)) in enumerate(kd_sets)
            nec_keys = [k for (k, d) in zip(nec_keys, nec_doors)
            ...
            for (sub_keys, sub_doors) in sub_kd_sets
                new_keys = union(sub_keys, nec_keys)
                new_doors = union(sub_doors, nec_doors)
                length(new_keys) < length(new_doors) && continue
...
"""
    mst_estim_cost(domain, state, agents, goals, room_labels, room_graph,
                   loc_objects, loc_graph; all_agents, all_doors)
