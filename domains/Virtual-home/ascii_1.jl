    ...
    for (y, row) in enumerate(rows)
        for (x, char) in enumerate(strip(row))
            ...
            elseif char == 'C'  # Door R = Red; B = Blue; Y = Yellow; E = Green; P = Purple
                ...
                if length(cabinets)==1
                    for i in 1:4
                        ...
                        push!(init, parse_pddl("(in $item $c)"))
                        append!(init, parse_pddl("(= (xloc $item) $x)", "(= (yloc $item) $y)"))
                    end
                end

                if length(cabinets)==2
                    for i in 1:4
                        item = Const(Symbol("plate$i"))
                        push!(init, parse_pddl("(in $item $c)"))
                        append!(init, parse_pddl("(= (xloc $item) $x)", "(= (yloc $item) $y)"))
                    end
                end

                if length(cabinets)==3
                    for i in 1:4
                        item = Const(Symbol("waterglass$i"))
                        push!(init, parse_pddl("(in $item $c)"))
                        append!(init, parse_pddl("(= (xloc $item) $x)", "(= (yloc $item) $y)"))
                    end
                    for i in 1:4
                        item = Const(Symbol("wineglass$i"))
                        push!(init, parse_pddl("(in $item $c)"))
                        append!(init, parse_pddl("(= (xloc $item) $x)", "(= (yloc $item) $y)"))
                    end
                end

                if length(cabinets)==6
                    for i in 1:4
                        item = Const(Symbol("cutleryfork$i"))
                        push!(init, parse_pddl("(in $item $c)"))
                        append!(init, parse_pddl("(= (xloc $item) $x)", "(= (yloc $item) $y)"))
                    end
                    for i in 1:4
                        item = Const(Symbol("cutleryknife$i"))
                        push!(init, parse_pddl("(in $item $c)"))
                        append!(init, parse_pddl("(= (xloc $item) $x)", "(= (yloc $item) $y)"))
                    end
                end

                if length(cabinets)==7
                    cuttingboard = Const(Symbol("cuttingboard1"))
                    chefknife = Const(Symbol("chefknife1"))
                    
                    items = [cuttingboard, chefknife]
            ...
            elseif char == 'F'  # Door R = Red; B = Blue; Y = Yellow; E = Green; P = Purple
                ...
                tomato=Const(Symbol("tomato1"))
                cucumber=Const(Symbol("cucumber1"))
                cupcake=Const(Symbol("cupcake1"))
                juice=Const(Symbol("juice1"))
                wine=Const(Symbol("wine1"))

                items = [tomato, cucumber, cupcake, juice, wine]

                for item in items
                    print(item)
                    push!(init, parse_pddl("(in $item $f)"))
                    append!(init, parse_pddl("(= (xloc $item) $x)", "(= (yloc $item) $y)"))
                end
